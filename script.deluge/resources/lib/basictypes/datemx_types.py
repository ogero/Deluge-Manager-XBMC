    ...
    def parse(cls, text):
        ...
        if result:
            if len(result.group(0)) != len(text.strip()):
                raise ValueError(
                    """Could not parse the entirety of %r as a TimeOfDay value, parsed %r"""%(text, result.group(0)))
...
class mxDateTimeDelta_DT(datatypedefinition.BaseType_DT):
    ...
    def format(cls, value):
        ...
        result = []
        for (attr, fmt, always) in (
        ('day', '%i', 0), ('hour', '%02i', 1), ('minute', '%02i', 1), ('second', '%0.2f', 0)):
